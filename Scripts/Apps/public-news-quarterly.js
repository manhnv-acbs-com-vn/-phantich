            ...
            $.map(response.ResponseData, function (item) {
                ...
                if (item.PSCode == "1043.1046") {
                    ...
                    $.map(JSON.parse(item.DataList), function (itemsub) {
                        ...
                        if (itemsub.Key == 'TitleDMCKQarter')
                            itemTitleDMCKQarter = lang === 'EN' ? itemsub.Name : itemsub.NameVN;
                        if (itemsub.Key == 'TitleCNQuarter')
                            itemTitleCNQuarter = lang === 'EN' ? itemsub.Name : itemsub.NameVN;
                        if (itemsub.Key == 'LinkTBCQuarter') {
                            itemTitleTBCQuarter = lang === 'EN' ? itemsub.Name : itemsub.NameVN;
                            itemLinkTBCQuarter = lang === 'EN' ? itemsub.Link : itemsub.Link;
                        }
                        ...
                        if (itemsub.Key == 'TitleTableMCKQuarter')
                            itemTitleTableMCKQuarter = lang === 'EN' ? itemsub.Name : itemsub.NameVN;
                    });
                }
            });
            $('#title-dmck-id').html(itemTitleDMCKQarter);

            $('#title-cnq-id').html(itemTitleCNQuarter +
                ' <div class="ptcp-widget w-auto d-inline-block">' +
                '   <div class="">' +
                '       <div class="acbs-dropdown-list cbtt-dropdown ">' +
                '          <select class="acbs-dropdown style2 selectpicker form-control-sm show-tick show-menu-arrow bs-select" id="sl-quarter" onchange="FilterData()">' +
                '          </select>' +
                '     </div>' +
                ' </div>' +
                '</div>');
...
function LoadDataDefault() {
    var htmlRegder = '';
    $.post(data_api_url + "/public-news-page/get-list-stock-month-quarter?strPeriod=QUARTER", function (response) {
        if (response.ResponseCode === '00_00') {
            if (Array.isArray(response.ResponseData)) {
                ...
                var myQuarter = [];
                $.map(response.ResponseData, function (item) {
                    myQuarter.push(item.ByQuarter);
                });
                var listQuarter = myQuarter.filter((v, i, a) => a.indexOf(v) === i);
                var indexQuarter = 0;
                $.map(listQuarter, function (itemQuarter) {
                    if (itemQuarter != "" && itemQuarter.length >= 6) {
                        indexQuarter = indexQuarter + 1;
                        if (indexQuarter == 1) {
                            $('#title-dmck-id').html('Báo cáo chứng khoán ký quỹ quý ' + itemQuarter.slice(4, 6) + ' năm ' + itemQuarter.slice(0, 4) + '');
                        }
                        $("#sl-quarter").append('<option value="' + itemQuarter + '">' + itemQuarter.slice(4, 6) + "/" + itemQuarter.slice(0, 4) + '</option>');
                    }                    
                });
                ...
                htmlRegder += '            <th scope="col" class="align-middle text-center">         ';
                htmlRegder += '                ' + itemTitleTableMCKQuarter + '                           ';
                htmlRegder += '            </th>                                 ';
                htmlRegder += '        </tr>                                             ';
                htmlRegder += '    </thead>                                              ';
                htmlRegder += '    <tbody>                                               ';
                var sttNumber = 0;
                var vlSlQuarter = $('#sl-quarter').val();
                var vlSlExchenge = $('#sl-Exchenge').val();
                var OPJData = response.ResponseData.filter(obj => {
                    return obj.ByQuarter === vlSlQuarter && obj.StockExchange === vlSlExchenge;
                });
                $('#title-tbc-id').html('<a href="javascript:void(0);" class="btn btn-file">' + ('Tải báo cáo quý ' + vlSlQuarter.slice(4, 6) + ' năm ' + vlSlQuarter.slice(0, 4)) + '</a>');

                $.map(OPJData, function (item) {
                    sttNumber = sttNumber + 1;
                    htmlRegder += '        <tr>                                              ';
                    htmlRegder += '            <td scope="row" class="align-middle">' + sttNumber + '</th>   ';
                    htmlRegder += '            <td class="align-middle text-center">' + (item.StockCodeEnd == null ? "" : item.StockCodeEnd) + '</td>             ';
                    htmlRegder += '        </tr>                                             ';
                    if (item.AttachFiles != "") {
                        var fileObj = item.AttachFiles != "" ? JSON.parse(item.AttachFiles) : null;
                        var fileLink = fileObj != null ? cdn_url + fileObj[0].Val : "";
                        $('#title-tbc-id').html('<a href="' + fileLink + '" download target="_blank"  class="btn btn-file">' + ('Tải báo cáo quý ' + vlSlQuarter.slice(4, 6) + ' năm ' + vlSlQuarter.slice(0, 4)) + '</a>');
...
function FilterData() {
    ...
    if (Array.isArray(OPJDataDefault))
    {
        ...
        $('#title-dmck-id').html('Báo cáo chứng khoán ký quỹ quý ' + vlSlQuarter.slice(4, 6) + ' năm ' + vlSlQuarter.slice(0, 4) + '');
        $('#title-tbc-id').html('<a href="javascript:void(0);" class="btn btn-file">' + ('Tải báo cáo quý ' + vlSlQuarter.slice(4, 6) + ' năm ' + vlSlQuarter.slice(0, 4)) + '</a>');
